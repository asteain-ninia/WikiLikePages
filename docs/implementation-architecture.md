...
- `generate-content-data.mjs`
    - `content/` 配下の Markdown ファイルを再帰的に収集する
...
- `tests/renderers.test.js`: HTML エスケープ、各描画関数の出力検証 (XSS 回避含む)、インライン Markdown 変換、インフォボックス描画、ノーティス描画、テンプレートモデル描画
...
2. ~~**Markdown インライン書式の HTML レンダリング**~~ — 実装済み。太字・斜体・コード・MediaWiki 書式をセクション本文で保持し、描画層で HTML に変換する
3. ~~**テンプレートプラグイン方式**~~ — 実装済み。`src/lib/templates/` にハンドラファイルを置くだけで自動認識される。`基礎情報 国` (インフォボックス) と `架空` (ノーティス) を実装済み
4. **画像・埋め込みの解決** — `![[画像.png]]` を `<img>` に、`![[ページ名]]` をトランスクルージョンに変換する
5. **callout 記法の表示** — `> [!note]` 等を装飾ブロックに変換する
6. **ブロックレベル書式** — リスト・テーブル・外部リンクの HTML 変換 (外部パーサ導入か自前実装かは未決: spec 14.2)
